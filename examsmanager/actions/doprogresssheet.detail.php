...
<title>:: <?php if($log_id != "?"): ?> - DRAFT COPY printed by - (<?php echo $user_username ?>)-> <?php endif; ?><?php echo $reg_no ?> - PROGRESS SHEET ::</title>
<style>
	.aa{
		font-weight:bold;
		text-align:right;
...
<a href="javascript:;" onClick="javascript:window.print()">Print</a>
    ...
    <td><img src="../../images/ich logo.jpg" width = 100 height = 120/></td><td align="center">
        <span style="font-family:'Arial Black', Gadget, sans-serif; font-weight:bolder; font-size:16px;"> EBONYI STATE UNIVERSITY</span><br>
		 ...
		 EXAMINATIONS UNIT<br>
        <u>STUDENTS' RESULTS DETAILS-DRAFT COPY</u></span>
    </td>
...
if ($reg_no == "" || !$sees){
	...
	$query = "SELECT std_firstname, std_middlename, std_lastname, std_dept, std_entry_year, std_origin_state, std_origin_lga, std_phone_no, std_birth_date, std_jambno FROM std_info WHERE std_reg_no = '$reg_no'" //SQL Statement to Generates std name
...
<table align="center" width="800" border="3" cellspacing="1" bordercolor="#333333" style="background:url(../../images/logo_faded.jpg); background-repeat:no-repeat; background-position:center; background-size:contain" >
    <tr bgcolor="#EEEEEE" >
	<td>SN</td>
	<td>NAME USED TO PUBLISH RESULT</td>
	<td>COURSE CODE</td>
	<td>COURSE TITLE</td>
	<td>LOAD</td>
	<td>TOTAL</td>
	<td>GRADE</td>
	<td>GPA & CGPA</td>
	<td>DATE UPLOADED/MEMO DETAILS</td>
	...
	if($num_rows_sess != 0){
		for ($i = 1; $i<=$num_rows_sess; $i++){
			...
			if($num_rows_sem != 0){
				...
				//emma echo "<td align = 'center' width='90' style='font-size:12px; font-weight:bolder'> CGPA = ". $cgpa ;
...
<?php
	if($num_rows_sess != 0){
		//emma echo "<p align='center' style='font-size:18px; font-weight:bolder'>Current Final CGPA Standing = $fcgpas - " . class_of_degree($fcgpas, $reg_no) . "</p>";
	}
}
