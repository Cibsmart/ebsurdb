...
$file = 'Reconcilaition_Oustanding_Courses.csv';
		...
		$sql11 = "SELECT std_name,std_dept FROM std_info WHERE std_reg_no = '$reg_no' ";
		$result11 = mysql_query($sql) or die(mysql_error());
		$num_result11 =  mysql_num_rows($result);
...
<?php
	...
	$query22 = "SELECT std_name, std_dept FROM std_info WHERE std_reg_no = '$reg_no'" //SQL Statement to Generates std name
;	$result22 = mysql_query($query22) // Execution of SQL Statement to Generates std name

;	$datas22 = mysql_fetch_array($result22);  // Getting std name from querry
	

	$std_name22 = stripslashes($datas22['std_name']);
	$std_dept22 = stripslashes($datas22['std_dept']);
...
<?php
		if ($num_results != 0){
			...
			for ($cnt_results = 0; $cnt_results < $num_results; $cnt_results++){
				...
				$description = 'FAILED.COURSE'; 

				$sn2 = $sn2;
				$course_code1 = $course_code;
				$course_title1 = $course_title;
				$credit_unit1 = $credit_unit;
				$total_score1 = $total_score;
				$l_grade1 = $l_grade;
				/////
				
				
				$memo_detail =  $sn2  . ',' . $description  . ',' . $std_name22  . ',' . $reg_no . ',' . $std_dept22 . ',' . $session . ',' . $semester . ',' . $sn2 . ',' . $course_code1 . ',' . $course_title1 . ',' . $credit_unit1 . ',' . $total_score1 . ',' . $l_grade1 . "\n" ;
