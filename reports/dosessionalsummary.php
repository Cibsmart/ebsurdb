...
include('../utilities/logging.php');
	...
	$act = "Viewed Sessional Summary for $dep - $sess - $level Level";
	logg($cid,$act);
...
<title>:: Departmental Sessional Summary - RDB (<?php echo $log_id ?>) EBSU ::</title>
<style>
	td{
		font-size:12px;
		font-family:arial;
...
<p align="center">
    ...
    <td><img src="../images/ich logo.jpg" width = 100 height = 110/></td><td align="center">
		...
		<?php echo "<div style='text-align:center; font-size:12px; font-weight:bold; font-family:arial black'>". $level . " LEVEL ". $sess. " SESSIONAL RESULT SUMMARY </div>"; ?>
    </td><td>
    </td>
    </table>
</p>
...
<?php
	...
	for ($zz = 0; $zz < $std_all; $zz++){
		...
		$Namee = $r_name["std_firstname"] . " " .$r_name["std_middlename"] ." ".$r_name["std_lastname"];
					...
					summarizingsessional($reg_no, $sess);
					$fcgpa = round($cgpa,3);
		if ($cib != 0 ){
			$fcgpa = round($fcgpa , 3);
