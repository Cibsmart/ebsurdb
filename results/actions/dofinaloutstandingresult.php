<?php 		session_start();
	...
	include('../../utilities/logging.php');
		...
		$act = "Viewed $reg_no - Final Outstanding Results/Courses";
		logg($cid,$act);
...
<a href="javascript:;" onClick="javascript:window.print()">Print</a>
    ...
    <td><img src="../../images/ich logo.jpg" width = 100 height = 120/></td><td align="center">
        ...
        <u>STUDENTS' FINAL OUTSTANDING COURSES</u></span>
    </td>
		...
		for ($i = 1; $i<=$num_rows_sess; $i++){
			...
			if($num_rows_sem != 0){
				for ($j = 1; $j <= $num_rows_sem; $j++){
					...
					outstanding_res_final($session, $semester, $reg_no);
				}
...
<?php
		...
		print_outstanding_final($reg_no,$fail_sess, $fail_sem, $ifeanyi);
		
	}
	
	$cib = unattempted_course_final($reg_no);
	$ccc = count($cib);
	//var_dump($cib);
?>
</table>
